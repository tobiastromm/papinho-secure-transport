...
#include <stdio.h>
#include <string.h>
static PST_RESULT credentials_from_der(const pst_u8*cert,pst_size cert_size,const pst_u8*key,pst_size key_size,pst_credentials**out){PST_CREDENTIAL_SOURCE source;PST_DER_ITEM leaf;memset(&source,0,sizeof(source));source.struct_size=sizeof(source);source.api_version=PST_API_VERSION;source.kind=PST_CREDENTIAL_SOURCE_CERT_CHAIN_DER_PKCS8_DER;leaf.data=cert;leaf.size=cert_size;source.certificate_chain=&leaf;source.certificate_count=1;source.private_key_der=key;source.private_key_der_size=key_size;return pst_credentials_create(&source,out);}
