...
#include <stdio.h>
#include <string.h>
static PST_RESULT custom_trust_from_der(const pst_u8*ca_der,pst_size ca_size,pst_trust**out){PST_TRUST_SOURCE source;PST_DER_ITEM anchor;memset(&source,0,sizeof(source));source.struct_size=sizeof(source);source.api_version=PST_API_VERSION;source.kind=PST_TRUST_SOURCE_CUSTOM_CA_DER;anchor.data=ca_der;anchor.size=ca_size;source.anchors=&anchor;source.anchor_count=1;return pst_trust_create(&source,out);}
