...
int main(void)
{
 ...
 CHECK((descriptor->capabilities&(PST_BACKEND_CAP_TLS_1_2|PST_BACKEND_CAP_ROLE_CLIENT|PST_BACKEND_CAP_NONBLOCKING|PST_BACKEND_CAP_BACKEND_WAIT|PST_BACKEND_CAP_ALPN_CLIENT|PST_BACKEND_CAP_LOCAL_IDENTITY|PST_BACKEND_CAP_PEER_CERT_AUTH|PST_BACKEND_CAP_CUSTOM_TRUST|PST_BACKEND_CAP_SYSTEM_TRUST|PST_BACKEND_CAP_PEER_INFO))==(PST_BACKEND_CAP_TLS_1_2|PST_BACKEND_CAP_ROLE_CLIENT|PST_BACKEND_CAP_NONBLOCKING|PST_BACKEND_CAP_BACKEND_WAIT|PST_BACKEND_CAP_ALPN_CLIENT|PST_BACKEND_CAP_LOCAL_IDENTITY|PST_BACKEND_CAP_PEER_CERT_AUTH|PST_BACKEND_CAP_CUSTOM_TRUST|PST_BACKEND_CAP_SYSTEM_TRUST|PST_BACKEND_CAP_PEER_INFO)&&!(descriptor->capabilities&PST_BACKEND_CAP_ROLE_SERVER),3);
 ...
 config.provider_selection.exact_provider_id="schannel";result=pst_connection_create(runtime,&config,&connection);CHECK(result==PST_RESULT_AUTH_FAILURE&&!connection,10);
 memset(&provider_info,0,sizeof(provider_info));provider_info.struct_size=sizeof(provider_info);provider_info.api_version=PST_API_VERSION;CHECK(pst_runtime_get_provider_info(runtime,0,&provider_info)==PST_RESULT_OK&&provider_info.initialized&&!strcmp(provider_info.provider_id,"schannel")&&(provider_info.capabilities&PST_CAP_TLS_1_2)&&!(provider_info.capabilities&PST_CAP_ROLE_SERVER),11);
 pst_trust_release(trust);pst_runtime_release(runtime);pst_backend_registry_reset();
 printf("SCHANNEL_BACKEND=PASS SPI3=PASS ROLE_CLIENT=ADVERTISED ROLE_SERVER=NOT_ADVERTISED TLS12=ADVERTISED TLS13=UNAVAILABLE_NOT_ADVERTISED INIT_CYCLES=100 NO_IDENTITY=AUTH_FAILURE\n");
