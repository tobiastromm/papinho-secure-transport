...
ASSERT_C(spi_descriptor_size,sizeof(PST_BACKEND_DESCRIPTOR)==36);
#endif
typedef struct candidate { const char *id; pst_u32 caps; pst_u32 client_caps; pst_u32 server_caps; int handshake_fails; } candidate;
static int fail(int n){printf("test_api2_spi3_contract: FAIL %d\n",n);return n;}
static const candidate providers[]={
 {"a",PST_CAP_ROLE_CLIENT|PST_CAP_TLS_1_2|PST_CAP_NONBLOCKING|PST_CAP_SYSTEM_TRUST|PST_CAP_PEER_NAME_VERIFY,PST_CAP_ROLE_CLIENT|PST_CAP_TLS_1_2|PST_CAP_NONBLOCKING|PST_CAP_SYSTEM_TRUST|PST_CAP_PEER_NAME_VERIFY,0,0},
 {"b",PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_2|PST_CAP_ALPN_SERVER|PST_CAP_NONBLOCKING|PST_CAP_CUSTOM_TRUST,0,PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_2|PST_CAP_ALPN_SERVER|PST_CAP_NONBLOCKING|PST_CAP_CUSTOM_TRUST,0},
 {"c",PST_CAP_ROLE_CLIENT|PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_2|PST_CAP_TLS_1_3|PST_CAP_ALPN_CLIENT|PST_CAP_ALPN_SERVER|PST_CAP_NONBLOCKING|PST_CAP_SYSTEM_TRUST|PST_CAP_CUSTOM_TRUST|PST_CAP_PEER_NAME_VERIFY,PST_CAP_ROLE_CLIENT|PST_CAP_TLS_1_2|PST_CAP_TLS_1_3|PST_CAP_ALPN_CLIENT|PST_CAP_NONBLOCKING|PST_CAP_SYSTEM_TRUST|PST_CAP_CUSTOM_TRUST|PST_CAP_PEER_NAME_VERIFY,PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_2|PST_CAP_TLS_1_3|PST_CAP_ALPN_SERVER|PST_CAP_NONBLOCKING|PST_CAP_CUSTOM_TRUST,1}
};
static int eligible(const candidate *p,pst_u32 role,pst_u32 required){pst_u32 caps=role==PST_CONNECTION_ROLE_CLIENT?p->client_caps:p->server_caps;return (required&~caps)==0;}
static const candidate *find(const char *id){int i;for(i=0;i<3;i++)if(!strcmp(providers[i].id,id))return &providers[i];return NULL;}
static const candidate *select_provider(pst_u32 mode,const char *exact,const char *const *ordered,pst_size count,pst_u32 role,pst_u32 required){pst_size i;const candidate*p;if(mode==PST_BACKEND_SELECTION_EXACT){p=find(exact);return p&&eligible(p,role,required)?p:NULL;}if(mode==PST_BACKEND_SELECTION_ORDERED){for(i=0;i<count;i++){p=find(ordered[i]);if(p&&eligible(p,role,required))return p;}return NULL;}if(mode==PST_BACKEND_SELECTION_AUTOMATIC){for(i=0;i<3;i++)if(eligible(&providers[i],role,required))return &providers[i];}return NULL;}
...
int main(void){const candidate*p;const char*order1[]={"a","b","c"};const char*order2[]={"c","b"};pst_u32 client12=PST_CAP_ROLE_CLIENT|PST_CAP_TLS_1_2; pst_u32 server12=PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_2; pst_u32 server_alpn=server12|PST_CAP_ALPN_SERVER;
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"a",NULL,0,PST_CONNECTION_ROLE_CLIENT,client12)==&providers[0],1);
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"a",NULL,0,PST_CONNECTION_ROLE_SERVER,server12)==NULL,2);
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"b",NULL,0,PST_CONNECTION_ROLE_SERVER,server_alpn)==&providers[1],3);
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"missing",NULL,0,PST_CONNECTION_ROLE_CLIENT,client12)==NULL,4);
 CHECK(select_provider(PST_BACKEND_SELECTION_ORDERED,NULL,order1,3,PST_CONNECTION_ROLE_SERVER,server_alpn)==&providers[1],5);
 p=select_provider(PST_BACKEND_SELECTION_ORDERED,NULL,order2,2,PST_CONNECTION_ROLE_SERVER,server_alpn);CHECK(p==&providers[2]&&p->handshake_fails,6);
 CHECK(select_provider(PST_BACKEND_SELECTION_AUTOMATIC,NULL,NULL,0,PST_CONNECTION_ROLE_CLIENT,client12)==&providers[0],8);
 CHECK(select_provider(PST_BACKEND_SELECTION_AUTOMATIC,NULL,NULL,0,PST_CONNECTION_ROLE_SERVER,server12)==&providers[1],9);
 CHECK(select_provider(PST_BACKEND_SELECTION_AUTOMATIC,NULL,NULL,0,PST_CONNECTION_ROLE_SERVER,PST_CAP_ROLE_SERVER|PST_CAP_TLS_1_3)==&providers[2],10);
 CHECK(select_provider(0,NULL,NULL,0,PST_CONNECTION_ROLE_CLIENT,client12)==NULL,20);
 CHECK(select_provider(PST_BACKEND_SELECTION_AUTOMATIC,NULL,NULL,0,PST_CONNECTION_ROLE_CLIENT,PST_CAP_EARLY_DATA)==NULL,21);
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"c",NULL,0,PST_CONNECTION_ROLE_CLIENT,client12|PST_CAP_SYSTEM_TRUST|PST_CAP_PEER_NAME_VERIFY)==&providers[2],22);
 CHECK(select_provider(PST_BACKEND_SELECTION_EXACT,"c",NULL,0,PST_CONNECTION_ROLE_SERVER,server12|PST_CAP_SYSTEM_TRUST)==NULL,23);
 CHECK(select_provider(PST_BACKEND_SELECTION_ORDERED,NULL,order2,2,PST_CONNECTION_ROLE_SERVER,server12|PST_CAP_SYSTEM_TRUST)==NULL,24);
 CHECK(select_provider(PST_BACKEND_SELECTION_AUTOMATIC,NULL,NULL,0,PST_CONNECTION_ROLE_SERVER,server12|PST_CAP_SYSTEM_TRUST)==NULL,25);
