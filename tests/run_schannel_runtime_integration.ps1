...
$serverOut=Join-Path $repo ("build\win32-x64-msvc-19.51-schannel\schannel-tls"+$TlsVersion+"-server.log")
$serverErr=Join-Path $repo ("build\win32-x64-msvc-19.51-schannel\schannel-tls"+$TlsVersion+"-server.err")
...
try {
    ...
    & build\win32-x64-msvc-19.51-schannel\test_schannel_runtime_integration.exe 127.0.0.1 $Port localhost $TlsVersion $Exchanges $CloseMode $trustKind $trustArg required $Alpn $certArg $keyArg OK
