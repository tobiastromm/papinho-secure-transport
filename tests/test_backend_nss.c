...
int main(void){const PST_BACKEND_DESCRIPTOR*d;PST_BACKEND_IO_RESULT io;pst_u32 ready,observed=0,expected,client,server,caps;void*backend=NULL,*runtime=NULL;
 pst_backend_registry_reset();d=pst_backend_nss_descriptor();CHECK(d&&!strcmp(d->id,"retrozilla-nss")&&pst_backend_validate(d)==PST_RESULT_OK,1);client=PST_CAP_TLS_1_2|PST_CAP_TLS_1_3|PST_CAP_ROLE_CLIENT|PST_CAP_LOCAL_IDENTITY|PST_CAP_PEER_CERT_AUTH|PST_CAP_PEER_NAME_VERIFY|PST_CAP_CUSTOM_TRUST|PST_CAP_PEER_INFO|PST_CAP_ALPN_CLIENT|PST_CAP_NONBLOCKING|PST_CAP_BACKEND_WAIT;server=PST_CAP_TLS_1_2|PST_CAP_TLS_1_3|PST_CAP_ROLE_SERVER|PST_CAP_LOCAL_IDENTITY|PST_CAP_PEER_CERT_AUTH|PST_CAP_PEER_CERT_OPTIONAL|PST_CAP_CUSTOM_TRUST|PST_CAP_PEER_INFO|PST_CAP_NONBLOCKING|PST_CAP_BACKEND_WAIT;expected=client|server;CHECK(d->capabilities==expected&&d->client_capabilities==client&&d->server_capabilities==server&&d->client_capabilities==0x00007ab7UL&&d->server_capabilities==0x0000727bUL&&d->capabilities==0x00007affUL&&!(server&(PST_CAP_SYSTEM_TRUST|PST_CAP_ALPN_SERVER|PST_CAP_PEER_NAME_VERIFY)),2);CHECK(d->vtable->wait&&d->vtable->peer_info_create&&d->metadata&&d->metadata->component_count==2,3);
 ...
 pst_backend_registry_reset();CHECK(pst_win32_register_retrozilla_nss()==PST_RESULT_OK,19);printf("NSS_SPI3_CLIENT=PASS ROLE_SERVER=ADVERTISED SERVER_SYSTEM_TRUST=NO SERVER_ALPN=NO SERVER_PEER_NAME=NO CLIENT_MASK=0x00007ab7 SERVER_MASK=0x0000727b AGGREGATE_MASK=0x00007aff READINESS=PASS CLOSE_CLASSIFICATION=PASS RECORD_SIZE_LIMIT_NORMALIZATION=PASS\n");printf("test_backend_nss: PASS\n");return 0;}
