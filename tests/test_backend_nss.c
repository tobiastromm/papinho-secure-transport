...
int main(void){const PST_BACKEND_DESCRIPTOR*d;PST_BACKEND_IO_RESULT io;pst_u32 ready,observed=0,expected,client,server,caps;void*backend=NULL,*runtime=NULL;
 ...
 pst_backend_registry_reset();CHECK(pst_win32_register_retrozilla_nss()==PST_RESULT_OK,19);printf("NSS_SPI3_CLIENT=PASS ROLE_SERVER=ADVERTISED SERVER_SYSTEM_TRUST=NO SERVER_ALPN=NO SERVER_PEER_NAME=NO READINESS=PASS CLOSE_CLASSIFICATION=PASS RECORD_SIZE_LIMIT_NORMALIZATION=PASS\n");printf("test_backend_nss: PASS\n");return 0;}
