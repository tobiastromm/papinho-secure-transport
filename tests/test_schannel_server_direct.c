...
int main(int argc,char **argv)
{
    ...
    else{CHECK(io.close_kind==PST_BACKEND_CLOSE_CLEAN);stage="reciprocal-close";do{result=d->vtable->shutdown_step(connection,&operation,&error);printf("SCHANNEL_SERVER_SHUTDOWN RESULT=%ld OP=%lu ERROR=%ld\n",(long)result,(unsigned long)operation,(long)error);fflush(stdout);CHECK(result==PST_RESULT_OK);CHECK(operation!=PST_BACKEND_OPERATION_FAILED);if(operation!=PST_BACKEND_OPERATION_COMPLETE)CHECK(wait_step(d,connection));}while(operation!=PST_BACKEND_OPERATION_COMPLETE);}
