...
#if defined(_WIN64)
A(options_size,sizeof(PST_BACKEND_CONNECTION_OPTIONS)==24);A(vtable_size,sizeof(PST_BACKEND_VTABLE)==160);A(descriptor_size,sizeof(PST_BACKEND_DESCRIPTOR)==56);
#else
A(options_size,sizeof(PST_BACKEND_CONNECTION_OPTIONS)==20);A(vtable_size,sizeof(PST_BACKEND_VTABLE)==84);A(descriptor_size,sizeof(PST_BACKEND_DESCRIPTOR)==36);
#endif
A(client_caps_after_vtable,offsetof(PST_BACKEND_DESCRIPTOR,client_capabilities)>offsetof(PST_BACKEND_DESCRIPTOR,vtable));
A(server_caps_after_client,offsetof(PST_BACKEND_DESCRIPTOR,server_capabilities)>offsetof(PST_BACKEND_DESCRIPTOR,client_capabilities));
