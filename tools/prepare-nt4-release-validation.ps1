...
$sdk=Join-Path $ValidationDirectory "win32-x86-vc6-retrozilla-nss"
foreach($path in @($source,(Join-Path $sdk "include\papinho_secure_transport.h"),(Join-Path $sdk "lib\win32-x86-vc6-retrozilla-nss\papinho_secure_transport.lib"))){if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw("Run validate-release-packages.ps1 first; missing "+$path)}}
...
$include=Join-Path $sdk "include";$lib=Join-Path $sdk "lib\win32-x86-vc6-retrozilla-nss";$envBat=Join-Path $repo "tools\vc6-env.bat"
...
Get-ChildItem -LiteralPath (Join-Path $sdk "runtime\win32-x86-vc6-retrozilla-nss") -File|Copy-Item -Destination $OutputDirectory -Force
