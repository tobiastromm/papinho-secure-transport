...
$runners=@{
 ...
 "run_tls13_mtls.bat"=@("@echo off",'if "%1"=="" goto usage',"set PST_NSS_TRACE_FILE=tls13-mtls-backend.log","test_nss_server_direct.exe %1 13 2 server.der intermediate.der server.pk8 root.der","if errorlevel 1 goto fail","echo PAPINHOSECURETRANSPORT M10 NT4 PACKAGE TLS 1.3 MTLS PASS","goto end",":usage","echo Usage: run_tls13_mtls.bat PORT","goto fail_end",":fail","echo PAPINHOSECURETRANSPORT M10 NT4 PACKAGE TLS 1.3 MTLS FAIL",":fail_end","verify other 2>nul",":end");
