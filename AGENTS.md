...
- Read `docs/build-modern-msvc.md`; do not manually rediscover MSVC/SDK paths unless its bootstrap fails.
- Use `tools\build-modern-msvc.bat clean`, then `tools\build-modern-msvc.bat test` from an ordinary shell.
- The bootstrap uses official `vswhere.exe` discovery and selects the newest complete C++ Build Tools installation. `PST_MODERN_VCVARS64` is the deliberate override.
- `Makefile.msvc` builds x64 with `/MD /W4` into `build\win64-modern-msvc`; zero warnings are required.
