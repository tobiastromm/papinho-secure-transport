...
The 0.6.3 candidate uses public API **2.1.0** and provider SPI **3.0**. It preserves the CLIENT/SERVER and scheduler contracts and normalizes RetroZilla NSS peer TLS-version metadata to the public PST TLS 1.2/TLS 1.3 vocabulary. The VC6 `/ML` and `/MD` CRT variants remain explicit, separately selectable artifacts.
...
- Current **0.6.3 / API 2.1.0 / SPI 3.0** candidate
...
The published 0.6.2 baseline passed real Windows NT 4.0 SP6 x86 and clean-machine validation for both VC6 `/ML` and `/MD`. A real PapinhoBrowser `/MD` integration consumed the `/MD` SDK and passed TLS 1.3. The 0.6.3 candidate preserves those artifact identities and requires targeted external recertification for its corrected NSS binaries. Platforms outside the documented validation matrix remain unvalidated.

## Distribution

The 0.6.3 candidate distribution consists of a source package and five static SDKs: explicit VC6/RetroZilla NSS `/ML` and `/MD` variants, Schannel, OpenSSL 3.5.8, and the optional Combined Schannel/OpenSSL target. The published 0.6.2 assets and historical unsuffixed VC6/NSS 0.6.1 package remain immutable.
