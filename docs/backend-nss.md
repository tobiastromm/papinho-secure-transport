...
The integration executable was compiled with VC6 `/W4`, but no real TLS handshake was executed in Phase 3 because the preserved project intentionally contains no certificate DB, private key, certificate fixture, or local TLS server, and this machine had no existing fixture-generation/NSS certutil toolchain. Creating public credentials/trust objects to bypass that boundary belongs to Phase 4. The historic harness remains evidence about the underlying runtime, not evidence that the new PST integration path completed TLS.
