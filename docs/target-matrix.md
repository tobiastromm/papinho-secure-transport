...
OpenSSL SERVER is implemented and validated on the existing modern x64 target with TLS 1.2/1.3. Schannel SERVER is implemented and validated there for TLS 1.2; TLS 1.3 SERVER and complete PST SERVER ALPN remain unadvertised factual limitations of the validated environment/provider behavior. RetroZilla NSS SERVER is implemented and validated for TLS 1.2/1.3 on the VC6 x86 target, including real NT4 SP6 execution, and advertises only its proven role-scoped capabilities. Cross-process network interoperability between x86 NSS and both x64 providers is validated. Same-process three-provider composition is not applicable to the current architecture-separated target matrix. This development status does not alter the frozen 0.4.0 release target claims above.
