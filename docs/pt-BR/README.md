...
A API 2.1 acrescenta uma peça importante sem mudar essa filosofia: o PST pode participar do scheduler da aplicação sem virar o dono da arquitetura do programa.

Um **wait-set portátil** reúne várias conexões PST através de tokens escolhidos pelo consumidor. No Win32, ele também pode observar fontes externas emprestadas, como o socket de um listener que continua pertencendo à aplicação.
...
Isso evita exigir um loop periódico de polling e também evita uma espera sequencial de `N × timeout`. Read e write continuam incrementais e bounded: a aplicação continua dona dos buffers, do restante ainda não enviado, de seus deadlines e de sua política de fairness.
...
A validação física final de NT4, clean machine e packages da futura 0.6.0 pertence à M10. Até ela terminar, a evidência de desenvolvimento não é apresentada como uma release já publicada.
...
No role SERVER atual, `SYSTEM_TRUST` e ALPN SERVER com a semântica completa do PST