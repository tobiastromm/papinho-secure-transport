...
## API 2.0 SERVER closure

SS-4 validates a TLS 1.2 Schannel SERVER with local leaf/intermediate/private-key identity, CUSTOM and SYSTEM clientAuth trust, disabled/optional/required peer-certificate modes, normalized peer information, incremental readiness, encrypted I/O, reciprocal close-notify and strict truncation. Required intermediates are reference-counted in `CurrentUser\\CA`; preexisting certificates are preserved and PST-inserted certificates are removed at the final normal reference. Abrupt process termination can leave crash residue and is documented as a limitation; PST does not risk deleting externally owned certificates during speculative recovery.

The SERVER capability mask deliberately excludes TLS 1.3 on the validated Windows environment, `PEER_NAME_VERIFY` (not applicable to client certificate authentication), and `ALPN_SERVER`. Schannel can prove required-match/server-preference cases, but its optional no-intersection behavior cannot satisfy the complete frozen PST SERVER ALPN contract. No ClientHello parser or API/SPI exception was added. Combined selection therefore filters Schannel before binding whenever SERVER ALPN is required.
