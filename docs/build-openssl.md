...
`Makefile.openssl.msvc` writes only to `build\win32-x64-msvc-19.51-openssl3`, compiles PST code with `/MD /W4`, links only the retained OpenSSL import library, and copies the exact retained DLLs next to the test executable. The target registers only `openssl`. Its public-header consumer includes only `papinho_secure_transport.h`.

The target implements OSSL-C TLS 1.2/TLS 1.3, nonblocking handshake, secure I/O, bounded readiness and shutdown. Run `tools\build-win32-x64-msvc-19.51-openssl3.bat runtime-integration` to build the functional client; the canonical PowerShell runner is `tests\run_openssl_runtime_integration.ps1`. Real evidence is written under ignored `build\phase-ossl-c`. Full public trust, hostname, ALPN, mTLS and peer information remain OSSL-D.
...
```bat
tools\build-win32-x64-msvc-19.51-openssl3.bat system-trust-integration
```
