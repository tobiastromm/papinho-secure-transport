...
Backend state and platform resources remain private behind a backend-neutral boundary. Phase 2 materializes the internal contract described in the historical Phase 2 SPI design record, and Phase 3 implements the first opt-in provider, `retrozilla-nss`, as documented in [backend-nss.md](backend-nss.md). Runtime selection policy remains deferred; neither the SPI nor backend/platform types are exposed publicly.
...
Phase 0.C resolves these design questions provisionally in the historical Phase 0.C runtime/API design record. That specification remains documentation, not a released public header or implementation.
