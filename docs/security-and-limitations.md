...
In the 0.5.0 development line, OpenSSL is the only implemented SERVER provider. Its client-certificate trust may use explicit CUSTOM trust or Windows SYSTEM trust. The Windows adapter applies server-auth usage plus hostname verification for CLIENT peers and client-auth usage without hostname semantics for SERVER peers. Successful certificate authentication does not authorize an application identity; authorization remains the consumer's responsibility. Raw EOF/reset before reciprocal `close_notify` remains `PST_RESULT_TRUNCATED` for SERVER as well as CLIENT.
