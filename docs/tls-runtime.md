...
Read/write results keep byte progress, progress state, clean close, truncation and fatal error separate. Peer info is available only when established and remains an owned independent snapshot. Shutdown is incremental; connection release is bounded and does not wait for peer `close_notify`.
