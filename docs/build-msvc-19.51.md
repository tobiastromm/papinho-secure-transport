...
`tools\msvc-env.bat` discovers the newest complete C++ installation through the official `vswhere.exe`. Set `PST_MSVC_19_51_VCVARS64` only to deliberately override discovery; the override path is validated before use.
...
The frozen 0.4.0 CRT choice is dynamic `/MD`; the release packages document the corresponding runtime requirements.
...
Outputs are isolated under `build\win32-x64-msvc-19.51-schannel`. The test target compiles and links the common PST core for x64, runs a public-header-only consumer, and runs the Schannel lifecycle/selection/ownership matrix and deterministic TLS buffer/readiness tests. `/W4` with zero warnings is required.
