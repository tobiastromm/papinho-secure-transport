...
Real closure gates passed with Python/OpenSSL, PST Schannel TLS 1.2 CLIENT, and PST RetroZilla NSS TLS 1.2/1.3 CLIENT. Combined EXACT, ORDERED and AUTOMATIC all bind OpenSSL for supported SERVER configurations; Schannel is skipped as ineligible before binding and no post-binding fallback occurs. A 20-cycle bounded success/failure/truncation repetition passed. The aggregate provider mask is `0x00007fff`, the union used for discovery. Eligibility is role-scoped: CLIENT advertises SYSTEM_TRUST and PEER_NAME_VERIFY; SERVER advertises CUSTOM_TRUST, SYSTEM_TRUST and ALPN_SERVER but not PEER_NAME_VERIFY. SS-3B proved SERVER SYSTEM trust at TLS 1.2/1.3, EXACT/ORDERED/AUTOMATIC selection, OPTIONAL and REQUIRED client authentication, client-auth EKU enforcement, normalized failure diagnostics and temporary CurrentUser-root cleanup.
