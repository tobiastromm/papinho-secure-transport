...
Current 0.6.0 status: OpenSSL implements API 2.1/SPI 3.0 CLIENT and SERVER TLS 1.2/TLS 1.3, CUSTOM_TRUST, Windows SYSTEM_TRUST where advertised, role-scoped ALPN and independent CLIENT SNI control. The OSSL-ST-A through OSSL-ST-D material below is retained as historical 0.4.0 implementation evidence.
...
third_party/openssl/prebuilt/win32-x64-msvc-19.51-openssl3/3.5.8/{include,lib,runtime}
third_party/openssl/prebuilt/win32-x64-msvc-19.51-openssl3/3.5.8/MANIFEST.sha256
build/win32-x64-msvc-19.51-openssl3
build/win32-x64-msvc-19.51-schannel-openssl3
```
...
- legacy: `[retrozilla-nss]`, `build/win32-x86-vc6-retrozilla-nss`;
- modern Schannel: `[schannel]`, `build/win32-x64-msvc-19.51-schannel`;
- modern OpenSSL: `[openssl]`, `build/win32-x64-msvc-19.51-openssl3`;
- future combined modern: `[schannel, openssl]`, `build/win32-x64-msvc-19.51-schannel-openssl3`.
...
The canonical failure/result matrix is preserved in the [archived OSSL-E record](codex/phase-history/openssl-failure-hardening.md). Fresh real tests cover plaintext and exact-version protocol failures, clean and truncated closure, established-session RST, fatal wait, incremental shutdown, 4 MiB backpressure and 100 repeated TLS 1.3 echoes. Existing OSSL-D/D2 identity, logging, lifecycle, same-runtime recovery, dual-runtime isolation, independent Schannel server, coexistence and selection proofs remain applicable. No production or public ABI change was required. OSSL-E and OSSL-F are complete.
## OSSL-F extension closure

The formal NSS/Schannel/OpenSSL comparison and SYSTEM_TRUST follow-up analysis are preserved in the [archived three-provider closure](codex/phase-history/openssl-extension-closure.md). The three providers remain first-class on their deliberate targets, common semantics agree across the exact capability intersection, and capability differences remain explicit. OSSL-F, the OpenSSL extension, and Phase 9 are complete; publication has not been performed.
...
The hardening matrix passed without a production-code change. Public TLS 1.2/1.3, wrong hostname, private SYSTEM rejection, private CUSTOM success, same-runtime and two-runtime recovery, adapter BOOL/status checks, diagnostics, logging, cleanup, selection, legacy separation, and all canonical builds are green. The public runner now performs a bounded TCP preflight so endpoint unavailability is classified as ENVIRONMENT_FAILURE; the default OpenSSL suite remains offline. Full policy and limitations are frozen in docs/openssl-system-trust.md. Phase 9 is complete; publication has not been performed.

## SS-3 OpenSSL SERVER

The 0.5.0 development provider implements API 2.0/SPI 3.0 SERVER with TLS 1.2/1.3, DER certificate-chain plus PKCS#8 local identity, DISABLED/OPTIONAL/REQUIRED client-certificate authentication with CUSTOM or Windows SYSTEM trust, server-ordered ALPN, incremental nonblocking handshake/read/write/wait/shutdown, peer snapshots and strict truncation. SYSTEM trust evaluates client certificates with the effective Windows chain engine and client-auth usage. The SERVER mask includes SYSTEM_TRUST but continues to exclude PEER_NAME_VERIFY, while the CLIENT mask includes both.

The role-scoped classification is factual: TLS_1_2, TLS_1_3, LOCAL_IDENTITY, PEER_CERT_AUTH, CUSTOM_TRUST, SYSTEM_TRUST, PEER_INFO, NONBLOCKING and BACKEND_WAIT are CLIENT_AND_SERVER; PEER_CERT_OPTIONAL and ALPN_SERVER are SERVER_ONLY; ALPN_CLIENT and PEER_NAME_VERIFY are CLIENT_ONLY. ROLE_CLIENT and ROLE_SERVER identify their respective masks. RESUMPTION and EARLY_DATA are not advertised.

Real closure gates passed with Python/OpenSSL, PST Schannel TLS 1.2 CLIENT, and PST RetroZilla NSS TLS 1.2/1.3 CLIENT. Combined EXACT, ORDERED and AUTOMATIC all bind OpenSSL for supported SERVER configurations; Schannel is skipped as ineligible before binding and no post-binding fallback occurs. A 20-cycle bounded success/failure/truncation repetition passed. Under API 2.1, the aggregate provider mask is `0x00067fff`, including CLIENT-only `SNI_CONTROL` and role-proven `GRACEFUL_SHUTDOWN`; it is the union used for discovery. Eligibility is role-scoped: CLIENT advertises SYSTEM_TRUST, PEER_NAME_VERIFY and SNI_CONTROL; SERVER advertises CUSTOM_TRUST, SYSTEM_TRUST and ALPN_SERVER but not PEER_NAME_VERIFY or CLIENT SNI control. SS-3B proved SERVER SYSTEM trust at TLS 1.2/1.3, EXACT/ORDERED/AUTOMATIC selection, OPTIONAL and REQUIRED client authentication, client-auth EKU enforcement, normalized failure diagnostics and temporary CurrentUser-root cleanup.
