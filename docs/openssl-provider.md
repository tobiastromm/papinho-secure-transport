...
The 0.5.0 development provider implements API 2.0/SPI 3.0 SERVER with TLS 1.2/1.3, DER certificate-chain plus PKCS#8 local identity, DISABLED/OPTIONAL/REQUIRED client-certificate authentication with CUSTOM trust, server-ordered ALPN, incremental nonblocking handshake/read/write/wait/shutdown, peer snapshots and strict truncation. OpenSSL SERVER SYSTEM_TRUST remains a factual limitation: SERVER accepts only explicit CUSTOM trust for authenticating client certificates. Its SERVER mask therefore excludes SYSTEM_TRUST and PEER_NAME_VERIFY, while its CLIENT mask includes both.

The role-scoped classification is factual: TLS_1_2, TLS_1_3, LOCAL_IDENTITY, PEER_CERT_AUTH, CUSTOM_TRUST, PEER_INFO, NONBLOCKING and BACKEND_WAIT are CLIENT_AND_SERVER; PEER_CERT_OPTIONAL and ALPN_SERVER are SERVER_ONLY; ALPN_CLIENT, SYSTEM_TRUST and PEER_NAME_VERIFY are CLIENT_ONLY. ROLE_CLIENT and ROLE_SERVER identify their respective masks. RESUMPTION and EARLY_DATA are not advertised.

Real closure gates passed with Python/OpenSSL, PST Schannel TLS 1.2 CLIENT, and PST RetroZilla NSS TLS 1.2/1.3 CLIENT. Combined EXACT, ORDERED and AUTOMATIC all bind OpenSSL for supported SERVER configurations; Schannel is skipped as ineligible before binding and no post-binding fallback occurs. A 20-cycle bounded success/failure/truncation repetition passed. The aggregate provider mask is `0x00007fff`, the union used for discovery. Eligibility is role-scoped: CLIENT advertises SYSTEM_TRUST and PEER_NAME_VERIFY; SERVER advertises CUSTOM_TRUST and ALPN_SERVER but not SYSTEM_TRUST or PEER_NAME_VERIFY. Consequently SERVER plus SYSTEM_TRUST is rejected before provider binding in EXACT, ORDERED and AUTOMATIC selection.
