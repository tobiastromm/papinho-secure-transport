...
The 0.5.0 development provider implements API 2.0/SPI 3.0 SERVER with TLS 1.2/1.3, DER certificate-chain plus PKCS#8 local identity, DISABLED/OPTIONAL/REQUIRED client-certificate authentication with CUSTOM trust, server-ordered ALPN, incremental nonblocking handshake/read/write/wait/shutdown, peer snapshots and strict truncation. OpenSSL SERVER SYSTEM_TRUST remains a factual limitation: SERVER accepts only explicit CUSTOM trust for authenticating client certificates and does not advertise a role-specific SYSTEM-trust claim.

Real closure gates passed with Python/OpenSSL, PST Schannel TLS 1.2 CLIENT, and PST RetroZilla NSS TLS 1.2/1.3 CLIENT. Combined EXACT, ORDERED and AUTOMATIC all bind OpenSSL for SERVER; Schannel is skipped as ineligible before binding and no post-binding fallback occurs. A 20-cycle bounded success/failure/truncation repetition passed. The provider mask is `0x00007fff`: TLS 1.2, TLS 1.3, CLIENT, SERVER, local identity, peer-certificate authentication/optional mode, CLIENT/SERVER ALPN, CUSTOM/SYSTEM trust, peer-name verification, peer info, nonblocking and backend wait. SYSTEM trust in that aggregate mask is the proven CLIENT capability and must not be interpreted as every role/capability cross-product.
