...
Status: Phase 9.E technical staging is defined, but public redistribution is blocked until the PST project license is selected.

## Distribution model

The first release should ship a source archive and three separate, release-only binary SDKs built as static libraries. A source-repository archive is not a substitute for the binary SDKs.
...
| `windows-x64-msvc-schannel-openssl-3.5.8` | Schannel then OpenSSL | x64 validation target | `/MD` | validation-only |
...
Combined remains validation-only. Capability-based choice between Schannel and OpenSSL is useful evidence, but its larger deployment and OpenSSL runtime obligation are not universally better. There is no post-selection fallback after TLS, authentication, or I/O failure.

## SDK tree

`tools/stage-release-sdk.ps1` recreates technical candidates under `dist/staging/0.3.0/<target-id>`:
