...
- Multiplexed readiness evolution on `feature/multiplexed-readiness`:
  ...
  - M10 NT4 / clean-machine / packaging / 0.6.0 release: complete; physical NT4 and clean-machine package validation, deterministic reproduction, publication verification and consumer handoffs passed.

- VC6 CRT artifact-identity evolution on `feature/vc6-md-crt-variant`:
  - M1 explicit `/ML` and `/MD` builds: complete.
  - M2 package identity, CRT provenance and extracted consumers: complete.
  - M3 package-only functional TLS parity: complete.
  - M4 external validation: complete; real NT4 and clean-machine `/ML`/`/MD` TLS parity passed, and real PapinhoBrowser `/MD` with the PST `/MD` SDK passed TLS 1.3.
  - M5 0.6.2 version freeze, final reproduction and publication gate: current; publication requires a separate explicit owner authorization after final hashes are reported.
