...
At this historical Phase 6.B package-preparation checkpoint, the package contained the rebuilt VC6 integration executable and deterministic echo fixture behavior. Modern-host TLS 1.2 and TLS 1.3 validation produced `WRITE=25 READ=25 CONTENT_MATCH=1`, while the server produced `RECV=25 SEND=25 CONTENT_MATCH=True`. A new real NT4 run was still required, and Phase 6 remained in progress.
...
At that historical Phase 7.B checkpoint, expected results were clean close for the first two modes, including `READ=29 CONTENT_MATCH=1` for data_then_close, and `FINAL_STATE=FAILED CLOSE_KIND=2 DIAG_RESULT=13 DIAG_OPERATION=7 PASS=1` for abrupt_close. The plan also required rerunning `run_tls12.bat` and `run_tls13.bat` against their normal echo fixtures. Phase 7.B then remained in progress until targeted real-NT4 evidence, shutdown-abort proof and closure audit were complete.
