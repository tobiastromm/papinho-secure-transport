...
struct pst_runtime { pst_runtime_provider providers[8];pst_size provider_count;pst_u32 initialized_count,connections; pst_internal_diagnostic diagnostic; pst_log_state log; };
...
void PST_CALL pst_runtime_release(pst_runtime *r){pst_u32 order;if(!r||r->connections)return;for(order=r->initialized_count;order;order--){pst_size i;for(i=0;i<r->provider_count;i++)if(r->providers[i].initialized&&r->providers[i].initialization_order==order){r->providers[i].descriptor->vtable->runtime_destroy(r->providers[i].runtime_state);r->providers[i].descriptor->vtable->shutdown(r->providers[i].backend_state);}}free(r);}
...
void PST_CALL pst_connection_release(pst_connection *c){if(!c)return;c->provider->descriptor->vtable->connection_destroy(c->backend_state);if(c->transport)c->transport->destroy(c->transport,1);pst_connection_config_snapshot_release(c->config);--c->runtime->connections;free(c);}
