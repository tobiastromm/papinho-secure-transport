...
struct pst_connection { pst_runtime *runtime;pst_runtime_provider *provider;pst_connection_config_snapshot *config;void *backend_state;pst_transport *transport;pst_wait_set *wait_set;pst_u32 state,role,pending_io,pending_interest,last_wait_ready,suppressed_interest;PST_RESULT terminal_result;int wait_observed; pst_internal_diagnostic diagnostic; };
...
PST_RESULT PST_CALL pst_connection_create_ex(pst_runtime *r,const PST_CONNECTION_CONFIG *cfg,pst_connection **out,PST_DIAGNOSTIC_INFO *diagnostic)
{
    ...
    if(r){const char *id=(result==PST_RESULT_OK)?(*out)->provider->descriptor->id:NULL;if(result==PST_RESULT_OK)pst_log_emit_facts(&r->log,PST_LOG_LEVEL_DEBUG,PST_LOG_EVENT_STATE_TRANSITION,PST_LOG_CATEGORY_CONNECTION,PST_RESULT_OK,PST_DIAGNOSTIC_OPERATION_CONNECTION,id,(*out)->role,PST_KNOWN_UNKNOWN,PST_KNOWN_UNKNOWN);else pst_log_emit(&r->log,PST_LOG_LEVEL_ERROR,PST_LOG_EVENT_CONNECTION_FAILURE,PST_LOG_CATEGORY_CONNECTION,result,PST_DIAGNOSTIC_OPERATION_CONNECTION,id);}
    if(diagnostic){export_result=pst_diagnostic_export_public(&context.diagnostic,diagnostic);if(export_result!=PST_RESULT_OK)return export_result;if(diagnostic->valid)diagnostic->normalized_result=result;}
    return result;
}
...
PST_RESULT PST_CALL pst_connection_shutdown(pst_connection *c,pst_u32 *op,PST_RESULT *e){PST_RESULT x,reported;const PST_BACKEND_DESCRIPTOR *d;if(!c||!op||!e||(c->state!=C_ESTABLISHED&&c->state!=C_SHUTTING)){if(op)*op=PST_OPERATION_FAILED;if(e)*e=PST_RESULT_INVALID_STATE;return PST_RESULT_INVALID_STATE;}d=c->provider->descriptor;progress_reset(c);c->state=C_SHUTTING;x=d->vtable->shutdown_step(c->backend_state,op,e);diagnostic_pull(d,c->backend_state,&c->diagnostic);reported=x!=PST_RESULT_OK?x:*e;if(*op==PST_OPERATION_COMPLETE){c->state=C_CLOSED;c->terminal_result=PST_RESULT_CLOSED;pst_log_emit_facts(&c->runtime->log,PST_LOG_LEVEL_INFO,PST_LOG_EVENT_CONNECTION_CLOSED,PST_LOG_CATEGORY_SHUTDOWN,PST_RESULT_OK,PST_DIAGNOSTIC_OPERATION_SHUTDOWN,d->id,c->role,PST_KNOWN_UNKNOWN,PST_KNOWN_TRUE);}else if(*op==PST_OPERATION_FAILED){c->state=C_FAILED;c->terminal_result=reported;log_connection_failure(c,reported,PST_DIAGNOSTIC_OPERATION_SHUTDOWN,PST_LOG_CATEGORY_SHUTDOWN);}else if(x!=PST_RESULT_OK){c->state=C_FAILED;c->terminal_result=x;log_connection_failure(c,x,PST_DIAGNOSTIC_OPERATION_SHUTDOWN,PST_LOG_CATEGORY_SHUTDOWN);}else pst_log_emit_facts(&c->runtime->log,PST_LOG_LEVEL_TRACE,PST_LOG_EVENT_OPERATION_PROGRESS,PST_LOG_CATEGORY_SHUTDOWN,PST_RESULT_OK,PST_DIAGNOSTIC_OPERATION_SHUTDOWN,d->id,c->role,PST_KNOWN_UNKNOWN,PST_KNOWN_UNKNOWN);return x;}
PST_RESULT PST_CALL pst_connection_try_release(pst_connection *c){pst_runtime *r;if(!c)return PST_RESULT_INVALID_ARGUMENT;if(c->wait_set)return PST_RESULT_INVALID_STATE;r=c->runtime;c->provider->descriptor->vtable->connection_destroy(c->backend_state);if(c->transport)c->transport->destroy(c->transport,1);pst_connection_config_snapshot_release(c->config);--r->connections;free(c);if(!r->connections&&r->release_requested)runtime_destroy(r);return PST_RESULT_OK;}
void PST_CALL pst_connection_release(pst_connection *c){if(c&&!c->wait_set)(void)pst_connection_try_release(c);}
