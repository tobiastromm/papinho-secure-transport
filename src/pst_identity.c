...
struct pst_config { pst_credentials *credentials; pst_trust *trust; char *hostname; pst_u32 peer, client; int frozen; };
...
PST_RESULT PST_CALL pst_config_create(pst_config **out){if(!out)return PST_RESULT_INVALID_ARGUMENT;*out=(pst_config*)calloc(1,sizeof(pst_config));return *out?PST_RESULT_OK:PST_RESULT_OUT_OF_MEMORY;}
...
void PST_CALL pst_config_release(pst_config *c){if(c){pst_credentials_release(c->credentials);pst_trust_release(c->trust);free(c->hostname);free(c);}}
