...
PST_RESULT PST_CALL pst_peer_info_get_summary(const pst_peer_info *v,PST_PEER_INFO_SUMMARY *s){pst_u32 n;if(!s||s->struct_size<PST_PEER_INFO_SUMMARY_MIN_SIZE)return PST_RESULT_INVALID_ARGUMENT;if(!version_ok(s->api_version))return PST_RESULT_INCOMPATIBLE_API;n=s->struct_size;memset(s,0,sizeof(*s));s->struct_size=n;s->api_version=PST_API_VERSION;if(!v)return PST_RESULT_INVALID_ARGUMENT;*s=v->summary;s->struct_size=n;s->api_version=PST_API_VERSION;return PST_RESULT_OK;}
