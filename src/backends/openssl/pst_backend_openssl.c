...
{unsigned char*leaf_der=NULL,*cursor;int leaf_size=i2d_X509(leaf,NULL);if(leaf_size<=0)goto done;leaf_der=(unsigned char*)malloc((size_t)leaf_size);if(!leaf_der)goto done;cursor=leaf_der;if(i2d_X509(leaf,&cursor)!=leaf_size){free(leaf_der);goto done;}trust_result=pst_ossl_windows_system_trust_evaluate(leaf_der,(pst_size)leaf_size,items,(pst_size)used,NULL);free(leaf_der);}
if(trust_result==PST_OSSL_WINDOWS_TRUST_CERTIFICATE_REJECTED){c->system_trust_result=PST_RESULT_AUTH_FAILURE;goto done;}if(trust_result!=PST_OSSL_WINDOWS_TRUST_OK)goto done;host_result=X509_check_host(leaf,c->hostname,0,0,NULL);if(host_result!=1){c->system_trust_result=host_result==0?PST_RESULT_PEER_NAME_MISMATCH:PST_RESULT_BACKEND_FAILURE;goto done;}c->system_trust_result=PST_RESULT_OK;X509_STORE_CTX_set_error(context,X509_V_OK);
